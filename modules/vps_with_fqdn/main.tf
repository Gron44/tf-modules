...
module vps {
  source = "git::https://github.com/Gron44/tf-modules.git//modules/YC/VPS?ref=v0.0.26"
...
module fqdn {
  ...
  source = "git::https://github.com/Gron44/tf-modules.git//modules/AWS/FQDN?ref=v0.0.26"


  route53_zone = var.route53_zone
  site_domain_name = lookup(var.dev, "count", 1) == 1 ? (
    format("%s.%s.%s.%s",
      lookup(var.dev, "fqdn_name", var.dev.name), 
      var.labels.task_name,
      var.student,
      var.route53_zone
    )) : (
    format("%s-%s.%s.%s.%s",
      lookup(var.dev, "fqdn_name", var.dev.name), count.index+1, 
      var.labels.task_name,
      var.student,
      var.route53_zone
    ))
